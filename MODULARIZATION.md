...
## 해결된 부채 (2026-05-30)
- ✅ **`state.project` Proxy 제거** — 모든 호출부를 `state.workspace`(공유) / `activeDoc()`(문서) 직접 접근으로 마이그레이션. 매직 Proxy 대신 export/▶Play/렌더 경계용 read-only `projectSnapshot()`만 남김. 소스에 `state.project` 0건. → 절차는 [REFACTOR-PROMPT.md](REFACTOR-PROMPT.md) §1.
- ✅ **TypeScript 점진 도입** — `tsconfig`(strict) + JSDoc 타입 + `// @ts-check` opt-in + turbo `typecheck` 태스크. 데이터 모델(`state.js`)·핵심 SDK(`tags.js`/`gid.js`)부터 타입 적용, 계약은 `packages/core/src/types.d.ts`. 파일 단위로 확장(전체 `.ts` 전환은 후속). → 절차·확장법은 [REFACTOR-PROMPT.md](REFACTOR-PROMPT.md) §2.

## 남은 후속 (선택)
- 타입 커버리지 확대: `// @ts-check`를 한 파일씩 추가하고 `apps/editor/tsconfig.json`의 `include`에 등록 → 최종적으로 `.js`→`.ts` 전환.
- 게임 앱(`apps/game`) typecheck 태스크 추가(pixi 타입 정합 작업 필요).
- 위 #1~#7(툴/패널 레지스트리 등)은 별개 작업으로 남음.
